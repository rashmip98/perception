...
def reconstruct3D(transform_candidates, calibrated_1, calibrated_2):
  ...
  for candidate in transform_candidates:
    ...
    for i in range(calibrated_1.shape[0]):
      lambdas[:,i] = np.linalg.pinv(np.hstack((calibrated_2.T[:,i].reshape(-1,1), -1*R@calibrated_1.T[:,i].reshape(-1,1))))@T
    #lambdas = np.linalg.pinv(np.hstack((calibrated_2.T, -1*R@calibrated_1.T)))@T
    #print(lambdas.shape)
